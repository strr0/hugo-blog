...
启动容器
```
docker run -p 6379:6379 --name appsmith-redis -d redis
```
...
启动容器
```
docker run -p 27017:27017 --name appsmith-mongodb -e MONGO_INITDB_DATABASE=appsmith -d mongo --replSet rs0
```
进入容器
```
...
APPSMITH_MONGODB_URI="你的Mongo实例地址（可不配置副本集）"
